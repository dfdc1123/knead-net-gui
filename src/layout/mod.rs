...
/// 对明显高于 median 的 seed (疑似 OOB / column_conflict 卡死) 打印其状态。
/// 阈值: cost > max(10.0 * median, 50_000.0)。给主流猜出 SA 为什么卡死。
#[allow(clippy::too_many_arguments)]
fn diagnose_expensive_seeds(
...
fn print_seed_cost_report(costs: &[f64], best_cost: f64, base_seed: u64) {
    if costs.is_empty() {
        return;
    }
