...
pub fn to_svg_fd_continuous(
    ...
    fd_positions: &[(f64, f64)],
) -> String {
    assert_eq!(placeable.len(), fd_positions.len());
    let n = placeable.len();

    // 建立 component_id → index 的反向映射
    let mut comp_to_idx: HashMap<ComponentId, usize> = HashMap::new();
    ...
    if let Some(pr) = board.power_rails() {
        for rail in pr.top.rows.iter().chain(pr.bottom.rows.iter()) {
            ...
            let _ = writeln!(
                out,
                r##"<text x="{}" y="{rail_y:.1}" font-size="11" font-weight="bold" fill="{color}" text-anchor="end" dominant-baseline="central">{label}</text>"##,
                MARGIN - 3.0,
                rail_y = rail_y + CELL_Y / 2.0,
                color = color,
                label = label,
            );
        }
    ...
    for net in circuit.nets() {
        ...
        for i in 0..comp_indices.len() {
            for j in (i + 1)..comp_indices.len() {
                ...
                let b = comp_indices[j];
                let x1 = MARGIN + fd_positions[a].0 as f32 * CELL_X + CELL_X / 2.0;
                let y1 =
                    MARGIN + (fd_positions[a].1 + y_offset as f64) as f32 * CELL_Y + CELL_Y / 2.0;
                let x2 = MARGIN + fd_positions[b].0 as f32 * CELL_X + CELL_X / 2.0;
                let y2 =
                    MARGIN + (fd_positions[b].1 + y_offset as f64) as f32 * CELL_Y + CELL_Y / 2.0;
                let _ = writeln!(
                    out,
                    r##"<line x1="{x1:.1}" y1="{y1:.1}" x2="{x2:.1}" y2="{y2:.1}" stroke="{color}" stroke-width="1.5" stroke-dasharray="6,4" opacity="0.35"/>"##
                );
            }
        }
    }
    ...
    for i in 0..n {
        ...
        let cx = MARGIN + fd_positions[i].0 as f32 * CELL_X + CELL_X / 2.0;
        let cy = MARGIN + (fd_positions[i].1 + y_offset as f64) as f32 * CELL_Y + CELL_Y / 2.0;
        let color = component_color(cid.raw());

        // 半透明大圆 + 实心小圆
        let _ = writeln!(
            out,
            r##"<circle cx="{cx:.1}" cy="{cy:.1}" r="14" fill="{color}" fill-opacity="0.12" stroke="{color}" stroke-width="1.5"/>"##
        );
        let _ = writeln!(
            out,
            r##"<circle cx="{cx:.1}" cy="{cy:.1}" r="3.5" fill="{color}"/>"##
        );
        let _ = writeln!(
            out,
            r##"<text x="{cx:.1}" y="{cyy:.1}" font-size="10" font-weight="bold" fill="{color}" text-anchor="middle" dominant-baseline="central">{ref_}</text>"##,
            cx = cx,
            cyy = cy + 15.0,
            color = color,
            ref_ = html_escape(comp.ref_())
    ...
    // 3) 画网格孔 (灰点, 帮助参照)
    for hole in board.holes() {
        let (hx, hy) = hole_px_from_pos(Position {
            x: hole.position.x,
            y: hole.position.y + y_offset,
        });
        let fill = match hole.region {
            Region::PowerRail => "#e5e7eb",
            Region::MainRail => "#f3f4f6",
        };
        let _ = writeln!(
            out,
            r##"<circle cx="{hx:.1}" cy="{hy:.1}" r="2" fill="{fill}" stroke="#d1d5db" stroke-width="0.5"/>"##
        );
    }
