# Layout SA release benchmark gate

基准日期：2026-07-15。固定 workload 是 `examples/inputs/h-bridge.kicad_pcb`、800 preset 40 列、spectral 初始化、10 seeds × 5000 attempts、base seed `0xCAFE_F00D`。命令：

```sh
RUSTFLAGS='--cfg profile_sa --cfg profile_cost' cargo run --release -- --quick --profile
```
