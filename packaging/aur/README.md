...
It is not published by GitHub Actions. The committed checksum placeholders intentionally prevent a release package from being submitted before real artifacts exist.
...
3. Download `SHA256SUMS` from the release and replace:
   - `REPLACE_WITH_DEB_SHA256`
   - `REPLACE_WITH_EXAMPLES_SHA256`
...
Do not use `SKIP` for release assets.
